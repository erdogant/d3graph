...
# %% Main
def d3graph(adjmat, df=None, node_name=None, node_color='#000080', node_color_edge='#000000', node_size=10,
            node_size_edge=1, width=1500, height=800, collision=0.5, charge=250, edge_width=None, edge_distance=None,
            directed=False, edge_distance_minmax=[None, None], title='d3graph', slider=None, savepath=None,
            savename='index', cmap='Paired', showfig=True, verbose=3):
    ...
    # Parameters
    config = _set_configurations(width, height, collision, charge, edge_distance_minmax, edge_distance, edge_width,
                                 directed, title, slider, savepath, savename, cmap, showfig, verbose)
    # Make node configurations
    df = _node_config(df, node_name, node_color, node_size, node_size_edge, node_color_edge, adjmat=adjmat,
                      cmap=config['cmap'])
    # Create dataframe from co-occurence matrix
    G, df = adjmat2G(adjmat, df, config['edge_distance_minmax'][0], config['edge_distance_minmax'][1],
                     config['edge_width'], verbose=config['verbose'])
    # Make slider
...
def _setup_slider(G, slider=None):
    ...
    edge_weight = []
    for item in tmplist:
        edge_weight = np.append(edge_weight, item['edge_weight'])
...
def _set_source_dir(config):
    ...
    if not os.path.isdir(config['path']) and config['path'] != '':
        if config['verbose'] >= 3:
            print(f"[d3graph] >Creating directory: {config['path']}")
...
def _write_index_html(config, json_data):
    ...
    if config['verbose'] >= 3:
        print(f'[d3graph] >Writing {index_file.absolute()}')
...
def _node_config(df, node_name, node_color, node_size, node_size_edge, node_color_edge, cmap='Paired', adjmat=None):
    ...
    elif isinstance(node_size_edge, (int, float)):
...
def adjmat2G(adjmat, df, edge_distance_min=None, edge_distance_max=None, edge_width=None, verbose=3):
    ...
    adjmat['edge_weight'] = adjmat['weight'].values.reshape(-1, 1) if isinstance(edge_distance_min, type(None)) else _normalize_size(adjmat['weight'].values.reshape(-1, 1), edge_distance_min, edge_distance_max)
    ...
    try:
        G = nx.from_pandas_edgelist(adjmat, 'source', 'target',
                                    ['weight', 'edge_weight', 'edge_width', 'source_label', 'target_label', 'source',
                                     'target'])
    except Exception:
        # Version of networkx<2
        G = nx.from_pandas_dataframe(adjmat, 'source', 'target',
                                     edge_attr=['weight', 'edge_weight', 'edge_width', 'source_label', 'target_label',
                                                'source', 'target'])

    # Add node information
    A = pd.concat([pd.DataFrame(adjmat[['target', 'target_label']].values),
                   pd.DataFrame(adjmat[['source', 'source_label']].values)], axis=0)
    ...
    if not df.empty:
        ...
        for col, i in itertools.product(df.columns, range(df.shape[0])):
            idx = df.index.values[i]
            if np.any(np.isin(getnodes, df.index.values[i])):
                G.nodes[idx][col] = str(df[col][idx])
            elif verbose >= 2: print('[d3graph] >WARNING: Not found')
    return (G, df)
...
# %%
def _set_configurations(width, height, collision, charge, edge_distance_minmax, edge_distance, edge_width, directed,
                        title, slider, savepath, savename, cmap, showfig, verbose):
    # curpath=os.path.realpath(sys.argv[0])
    curpath = os.path.dirname(os.path.abspath(__file__))

    config = {'verbose': verbose, 'path': savepath, 'network_width': width, 'network_height': height, 'network_title': title, 'network_charge': charge * -1, 'network_collision': collision, 'edge_distance': edge_distance, 'edge_width': edge_width, 'edge_distance_minmax': edge_distance_minmax, 'directed': directed, 'showfig': showfig, 'd3_library': os.path.abspath(os.path.join(curpath, 'd3js/d3.v3.js'))}
